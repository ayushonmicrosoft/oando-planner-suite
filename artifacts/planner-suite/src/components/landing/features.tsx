      ...
      <div className="max-w-[1200px] mx-auto px-5">
        ...
        >
          ...
          <h2 className="text-[32px] md:text-[48px] font-bold text-white mb-5 tracking-[-0.02em]">
            ...
            <span className="text-white/35"> Any Office Space</span>
          </h2>
          <p className="text-[17px] text-white/35 max-w-[560px] mx-auto leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((f, i) => (
            <motion.div
              key={f.title}
              className={`group relative rounded-2xl border border-white/[0.06] bg-white/[0.02] p-6 ${f.borderHover} hover:bg-white/[0.04] transition-all duration-500 hover:shadow-lg ${f.glowColor}`}
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.06, duration: 0.5 }}
            >
              <div className={`w-11 h-11 rounded-xl bg-gradient-to-br ${f.gradient} flex items-center justify-center mb-5 transition-transform duration-500 group-hover:scale-110`}>
                <f.icon className={`w-5 h-5 ${f.iconColor}`} strokeWidth={1.8} aria-hidden="true" />
              </div>
              <h3 className="font-semibold text-white text-[15px] mb-2.5 tracking-[-0.01em]">{f.title}</h3>
