        ...
        >
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <div className="text-3xl md:text-4xl font-bold text-[#1F3653]">{s.value}</div>
        ...
        <div className="max-w-[800px] mx-auto mb-16">
          <div className="bg-card border rounded-xl p-8 md:p-12 shadow-sm min-h-[240px]">
            <div className="flex items-start gap-3 mb-6">
              <svg className="w-10 h-10 text-[#1F3653]/30 shrink-0" fill="currentColor" viewBox="0 0 24 24">
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all ${
                  i === active ? "bg-[#1F3653] w-8" : "bg-border hover:bg-muted-foreground"
                }`}
