...
export default function Landing() {
  ...
  const handleGetStarted = () => setLocation("/sign-in");
      ...
      <nav className="sticky top-0 z-50 bg-[#0B1324]/95 backdrop-blur-md border-b border-white/10">
        <div className="max-w-[1200px] mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <img src={`${BASE}logo-v2-white.webp`} alt="One&Only" className="h-7 w-auto" />
          </div>
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setLocation("/sign-in")} className="text-sm text-white/70 hover:text-white transition-colors">
              Sign In
            </button>
            <button
